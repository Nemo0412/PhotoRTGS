...
void GaussianModel::deleteRandomGaussians(int num_to_delete)
{
    int total_gaussians = this->getXYZ().size(0);
    if (total_gaussians <= num_to_delete) {
        return; // Don't delete all Gaussians
    }
    
    // Create a random mask to select Gaussians to delete
    auto indices = torch::randperm(total_gaussians, torch::TensorOptions().device(device_type_));
    auto delete_mask = torch::zeros({total_gaussians}, torch::TensorOptions().device(device_type_).dtype(torch::kBool));
    delete_mask.index_put_({indices.index({torch::indexing::Slice(0, num_to_delete)})}, true);
    
    // Use the existing prunePoints method with the delete mask
    this->prunePoints(delete_mask);
}
