...
# Photo-SLAM RTGS Runner Script
# Usage: ./run_rtgs.sh [dataset_type] [scene_name] [dataset_path] [results_path] [options]
...
PHOTOSLAM_RTGS_DIR="$SCRIPT_DIR/PhotoSLAM_RTGS"

# Check if PhotoSLAM_RTGS directory exists
if [ ! -d "$PHOTOSLAM_RTGS_DIR" ]; then
    echo "Error: PhotoSLAM_RTGS directory not found at $PHOTOSLAM_RTGS_DIR"
    exit 1
fi
...
EXECUTABLE_PATH="$PHOTOSLAM_RTGS_DIR/bin/$EXECUTABLE"
if [ ! -f "$EXECUTABLE_PATH" ]; then
    echo "Error: Executable not found: $EXECUTABLE_PATH"
    echo "Please build Photo-SLAM RTGS first:"
    echo "  cd $PHOTOSLAM_RTGS_DIR && ./build.sh"
    exit 1
fi

# Check if config files exist
ORB_CONFIG_PATH="$PHOTOSLAM_RTGS_DIR/$ORB_CONFIG"
GAUSSIAN_CONFIG_PATH="$PHOTOSLAM_RTGS_DIR/$GAUSSIAN_CONFIG"
...
VOCAB_PATH="$PHOTOSLAM_RTGS_DIR/ORB-SLAM3/Vocabulary/ORBvoc.txt"
if [ ! -f "$VOCAB_PATH" ]; then
    echo "Error: ORB vocabulary file not found: $VOCAB_PATH"
    exit 1
fi

echo "Running Photo-SLAM RTGS..."
...
# Change to PhotoSLAM_RTGS directory and run
cd "$PHOTOSLAM_RTGS_DIR"
